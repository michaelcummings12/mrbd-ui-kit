...
function Spinner({ className, ...props }: React.ComponentProps<"svg">) {
	return (
		<svg role="status" aria-label="Loading" viewBox="0 0 100 100" fill="none" className={cn("animate-spin", className)} {...props}>
			<circle
				cx="50"
				cy="50"
				r="33"
...
function Dots({ size, color }: { size: number; color: string }) {
	...
	return (
		<div className="flex items-center justify-center" style={{ width: totalWidth, height: size, gap }}>
			{[0, 1, 2].map((i) => (
				<div
					key={i}
					style={{
...
export function LoadingIndicator({
	...
	return (
		<div role="status" aria-label={label} className={`inline-flex items-center justify-center ${className ?? ""}`.trim()}>
